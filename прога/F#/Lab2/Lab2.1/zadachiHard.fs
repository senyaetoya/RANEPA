...
let zad2() =
    let personnel = [('f',28);('f',43);('m',25);('m',37);('m',48);('m',60)] 
    let women, men = List.partition (fun (gender, _) -> gender = 'm') personnel
    let averageAge x = List.averageBy (fun (_, age) -> double age) x
    printfn "Средний возраст женщин: %A, средний возраст мужчин: %A" (averageAge(women)) (averageAge(men))   
zad2()
